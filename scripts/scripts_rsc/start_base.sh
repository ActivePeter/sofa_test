...
# gc option
mkdir -p "${HOME}/logs"
JAVA_OPTS="$JAVA_OPTS -XX:+DisableExplicitGC -XX:+PrintGCDetails -Xloggc:${HOME}/logs/registry-${REGISTRY_APP_NAME}-gc.log -verbose:gc"
...
JAVA_OPTS="$JAVA_OPTS -XX:-OmitStackTraceInFastThrow -XX:ParallelGCThreads=4"
